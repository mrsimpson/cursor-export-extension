...
// 显示状态
async function showStatus(callback) {
...
// 检测关键短语
function detectKeyPhrases(words) {
    const phrases = [];
...
function formatSQL(code) {
    try {
        return (0, sql_formatter_1.format)(code, {
            language: 'sql',
            tabWidth: 2,
            keywordCase: 'upper',
...
// 从预定义语料库获取IDF值
function getCorpusIDF(word) {
    // 常用词IDF值缓存
    const commonWordsIDF = {
        // 软件架构相关
...
// 计算位置加权
function calculatePositionBoost(positions, totalWords) {
    let boost = 1.0;
    // 标题位置 (前5%)
    if (positions.some(pos => pos < totalWords * 0.05))
        boost *= 2.0;
    // 开头位置 (5-15%)
    else if (positions.some(pos => pos < totalWords * 0.15))
        boost *= 1.5;
    // 结尾位置 (后15%)
    else if (positions.some(pos => pos > totalWords * 0.85))
        boost *= 1.3;
    // 分散度加权
