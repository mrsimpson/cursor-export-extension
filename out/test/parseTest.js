...
exports.parseMarkdownElements = parseMarkdownElements;
...
function detectTags(conversation) {
    const tags = new Set();
    // 添加基于意图的标签
...
function findRelatedConversations(currentConv, allConvs) {
    const related = [];
    allConvs.forEach(conv => {
        ...
        const reasons = [];
        // 检查主题相似度
        const commonTopics = currentConv.metadata.topics.filter(t => conv.metadata.topics.includes(t));
        ...
        // 检查标签相似度
        const commonTags = currentConv.metadata.tags.filter(t => conv.metadata.tags.includes(t));
...
function calculateDetailedStatistics(messages) {
    const stats = {
        ...
        if (msg.type === 'user') {
            ...
            if (msg.metadata?.toolCalls?.length) {
                msg.metadata.toolCalls.forEach(call => {
                    ...
                    if (call.timestamp) {
                        const nextCall = msg.metadata?.toolCalls?.find(c => c.timestamp !== undefined &&
    ...
    try {
        ...
        Object.entries(data).forEach(([key, value]) => {
            try {
                ...
                // 处理消息时间戳和中间状态
                const messages = conv.conversation.map((msg, index) => {
                ...
                // 计算最后更新时间
                const lastTimestamp = Math.max(...messages.map((m) => m.timestamp), ...messages.flatMap((m) => (m.metadata?.toolCalls || []).map((c) => c.timestamp || 0)));
                conversations.push({
                    id: key.split(':')[1],
                    messages,
                    metadata: {
...
async function exportConversations(conversations) {
...
async function findActiveConversations(db) {
    return new Promise((resolve, reject) => {
        ...
        `;
        db.all(editorQuery, [], (err, editorRows) => {
            if (err) {
            ...
            `;
            db.all(workspaceQuery, [], (err, workspaceRows) => {
                if (err) {
                ...
                db.all(recentQuery, [], (err, recentRows) => {
                    ...
                    // 按时间戳排序并去重
                    const uniqueLocations = Array.from(new Map(locations
...
// 修改 main 函数中的相关部分
async function main(useTestData = false) {
    let currentDb;
    try {
        let data;
        if (useTestData) {
