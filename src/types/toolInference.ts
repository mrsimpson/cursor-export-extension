...
// 工具模式定义
export const toolPatterns: ToolPattern[] = [
    {
        tool: 'edit_file',
        confidence: 0.95,
        patterns: [
...
// 工具匹配函数
export function matchToolPattern(text: string, pattern: ToolPattern): ToolMatch | null {
...
// 计算置信度
export function calculateConfidence(match: ToolMatch, pattern: ToolPattern): number {
    ...
    // 基于动作语义强度调整置信度
    const strongActionWords: Record<string, RegExp> = {
...
function isExplanation(text: string, context: { tool?: string }): boolean {
    const explanationPatterns: Record<string, RegExp> = {
